...
```
§0     L  38–85    3.6 KB  Q7 — Dialog, and what Accordion would not have exercised
...
§1.1   L  92–105   0.8 KB  Row 1 — `hidden` vs the recipe's `display`: a global rule, not a per-component one
...
§2.6   L 258–273   0.8 KB  The controlled-mode predicate, and the parts it touches
§3     L 274–428   8.4 KB  `anatomy` → part components
...
§7.1   L 688–707   1.4 KB  The split, drawn exactly
...
§9.3   L 917–935   0.9 KB  How the definition of done has to say it
§10    L 936–1042  6.8 KB  SSR and hydration
§10.1  L 944–960   1.1 KB  `_hk` — Solid matches server and client nodes by position
§10.2  L 961–989   1.9 KB  The `children()` decision procedure
...
§11    L1043–1574 20.7 KB  Dialog, worked fully through
§11.1  L1048–1070  1.1 KB  File layout
§11.2  L1071–1102  1.2 KB  `dialog-context.ts`
...
§12    L1575–1606  3.2 KB  Assumptions this blueprint rests on, and the gate for each
...
§12.3  L1595–1606  1.2 KB  New assumptions P5 introduces
§13    L1607–1623  3.8 KB  What P5 changes — re-plan P6 before P6 is written
§14    L1624–1642  2.3 KB  What P5 could not act on
```
...
§1    L 72–274 13.7 KB  `/docs/styling/static-extraction`
§1.1  L 76–97   1.6 KB  The frame, and why it is not Chakra's
§1.2  L 98–200  6.8 KB  Section order
...
§4.5  L466–474  0.3 KB  What this tier renders
§5    L475–559  6.1 KB  `/docs/get-started/migration` — coming from Chakra UI (React)
...
§6    L560–625  4.1 KB  `/docs/theming/chakra-config` — the config function
§6.1  L575–617  3.0 KB  Section order
...
§8    L733–883  8.8 KB  The component page — one template, applied 111 times
...
§8.10 L845–862  1.2 KB  `### Dynamic values` — only on the eight implementations `roadmap.md` §3.1 marks
...
§1   L 66–207  9.8 KB  The app
§1.1 L 68–91   1.7 KB  The stack, and the second job it has to do
§1.2 L 92–108  1.7 KB  The config, and the four knobs that are not preferences
...
```
§0     L 22–76   4.5 KB  Summary — what this repo owes, to whom
§0.1   L 58–76   1.3 KB  The reference checkouts these claims were verified against
§1     L 77–303 14.5 KB  License compatibility
§1.1   L 79–99   1.2 KB  The whole dependency graph is MIT
§1.2   L100–184  5.5 KB  The one Apache-2.0 route — real, analysed, and closed at P2
...
§2.6   L384–411  1.7 KB  Checklist for a new derivative file
§3     L412–757 21.9 KB  Trademark
...
§4.5   L901–926  1.3 KB  What the choice unblocked, and what is still outstanding
§5     L927–957  3.3 KB  Tracking upstream over time
§6     L958–980  3.3 KB  Open items
```

## `plan.md`

```
§0    L  29–122   7.6 KB  The governing constraint: no runtime CSS-in-JS
...
§0.4  L  85–112   3.6 KB  What it costs — the parity delta, with its cause named
§0.5  L 113–122   0.4 KB  Consequence for the reference policy
§1    L 123–301  11.0 KB  Q2 — how consumers get CSS for variants their source never writes
...
§2    L 302–402   5.8 KB  Q4 — the style-props API
...
§3    L 403–645  14.6 KB  The styling layer
§3.1  L 405–431   2.3 KB  `panda.config.ts`, knob by knob
...
§5    L 742–860   7.3 KB  The package graph
...
§6    L 861–890   2.3 KB  Presence — a package-graph decision, not a component detail
§7    L 891–979   5.5 KB  Color mode, direction, locale, environment
...
§9    L1011–1036  1.3 KB  Dev-time resolution and codegen ordering
§10   L1037–1078  2.6 KB  Workstream B — the non-machine surface
§11   L1079–1109  2.9 KB  Assumptions this architecture rests on
§11.1 L1081–1093  1.5 KB  `brief-plan` §8 assumptions P3 depends on
§11.2 L1094–1109  1.4 KB  New assumptions P3 introduces
§12   L1110–1130  4.0 KB  What P3 changes in the plan — re-plan P4 and P5 against this
...
§2.5  L 231–272   2.5 KB  `renderStyled` — 104 lines, and the four details that cost something to learn
...
§3    L 338–480   9.5 KB  The spike — `spike/zag-solid`, 7 commits to `ef91b69`
...
§3.3  L 384–427   2.9 KB  The ten axes, as finally scored
...
§7    L 739–833   5.2 KB  The `inert` gap, re-measured at our target version
§8    L 834–969   9.8 KB  Two rules this evidence sets, and one boundary they draw
§8.1  L 836–877   3.0 KB  The methodology rule — measure the dependency, do not reason about its source
§8.2  L 878–937   4.8 KB  The port rule — no a11y beyond Zag, nothing Chakra does not have
§8.3  L 938–969   1.9 KB  Presence, drawn precisely — the one carry-over that becomes a build
§9    L 970–1057  8.0 KB  Carry-over verdicts with attribution status
...
§10   L1058–1140  9.2 KB  What P2 contradicts — for re-planning before P3
§10.1 L1063–1083  3.1 KB  The port rule — the widest change, and it came from the P2 gate, not the checkout
§10.2 L1084–1100  3.2 KB  Measurement contradictions
§10.3 L1101–1115  1.1 KB  Structural facts, not contradictions, that later phases should not assume away
...
```
§1    L  38–254  11.4 KB  The three enumerations, measured
...
§1.3  L 114–242   7.1 KB  The preset — 18 recipes + 56 slot recipes, and three things the count hides
§1.4  L 243–254   0.6 KB  `brief-plan` §8 assumption 2 is closed
§2    L 255–392   7.8 KB  The 56-vs-51 reconciliation
...
§2.4  L 327–361   2.3 KB  The seventeen machines with no slot recipe — and the thirteen with no Chakra component
§2.5  L 362–392   1.8 KB  Four components whose recipe key resolves to nothing — in Chakra too
§3    L 393–468   5.0 KB  The matrix — columns, and the one that needs re-pointing
§3.1  L 411–468   3.9 KB  What the CIJ column means, and what it must not become
§4    L 469–630  14.3 KB  The matrix
§4.1  L 471–520   6.3 KB  Machine components — 45 rows
§4.2  L 521–540   1.4 KB  Multi-part components with no machine — 15 rows
§4.3  L 541–566   2.0 KB  Atomic-recipe components — 21 rows
...
§5    L 631–774   8.8 KB  Exclusions, one reason each
...
§9    L 958–1029  7.6 KB  The build order
§9.1  L 964–996   3.5 KB  The probe phase — unchanged, plus one insertion
§9.2  L 997–1018  3.3 KB  The batches
...
§12   L1099–1131  4.7 KB  Assumptions — closed, opened, left open
...
§13   L1132–1157  6.2 KB  What P6 changes — re-plan P7–P9 against this
§14   L1158–1193  3.7 KB  What P6 could not act on
```

## `testing.md`

```
§0   L  37–58    1.3 KB  The division of labour with `definition-of-done.md`
...
§1.5 L 123–138   0.9 KB  Hydration round-trip fixtures
...
§3   L 255–449  11.3 KB  The generated-CSS coverage check — `check:css-coverage`
...
§3.4 L 331–359   1.2 KB  The allow-list — shape
§3.5 L 360–390   1.5 KB  The configuration canary — failing loudly on a `hash`/`prefix` mismatch
§3.6 L 391–425   1.7 KB  Failure output
...
§4.2 L 470–503   1.9 KB  The allowance register — shape
§4.3 L 504–518   0.8 KB  What the register's contents are, and where they live
§5   L 519–597   4.5 KB  The §0 checks — two checks, not one grep
...
§6   L 598–707   6.5 KB  The lint rules
§6.1 L 606–627   1.3 KB  Rule 1 — `style-prop-static-value` (route 3 used as route 1)
§6.2 L 628–661   2.0 KB  Rule 2 — `require-style-source` (the style-prop collision)
...
§6.5 L 684–707   1.4 KB  The answer to `docs-plan.md` §2 **D-2**
§7   L 708–819   7.0 KB  Storybook — the local playground
...
§1   L  60–168   7.9 KB  Q6 — copied or re-derived, answered twice
§1.1 L  78–101   2.0 KB  The fork — **copied, from `ef91b69`, all fourteen files**
§1.2 L 102–128   2.2 KB  The hope-ui-owned code milestone one needs — **copied, per item, per ref**
§1.3 L 129–168   2.7 KB  The rejected alternative for the fork — re-derive from upstream `1.43.0`
§2   L 169–253   4.6 KB  The upstream file set, exactly
§2.1 L 171–190   1.1 KB  Eight files, 594 lines, at `@zag-js/solid@1.43.0`
§2.2 L 191–216   1.3 KB  The fork is **7 + 7** — one file dropped, and why
§2.3 L 217–233   0.8 KB  v1 and v2 share the adapter **byte for byte** — a stronger claim than the plan's
§2.4 L 234–253   1.3 KB  What that means for a future v2 move — the move is adapter-*free*, not merely adapter-local
§3   L 254–339   6.6 KB  The public API, and parity with the react/vue/svelte/preact/vanilla siblings
§3.1 L 256–271   1.0 KB  The surface
§3.2 L 272–291   1.2 KB  Parity with the siblings
...
§4   L 340–430   8.4 KB  The four defects, at their current state
§4.1 L 345–358   2.1 KB  The four, as they stand
§4.2 L 359–384   1.6 KB  What P5 must **not** inherit — B5's idiom no longer exists
§4.3 L 385–430   4.5 KB  The predicted rows, re-checked — and three real deltas against `1.43.0`
§5   L 431–553   8.0 KB  The §0 compliance audit of the Zag machine set
...
§6   L 554–693  10.6 KB  The test plan — proving the adapter correct with no component involved
...
§7   L 694–806   5.9 KB  The attribution checklist — lands in the same commit as the code
...
§9   L 928–980   5.3 KB  Assumptions this milestone rests on, and their gates
§9.1 L 930–944   1.8 KB  `brief-plan` §8 assumptions
§9.2 L 945–969   2.1 KB  What `prior-art.md` §10.5 gets re-measured here, and what does not
§9.3 L 970–980   1.3 KB  New assumptions P4 introduces
§10  L 981–1008  5.1 KB  What P4 changes — re-plan P5 before P5 is written
§11  L1009–1022  1.7 KB  What P4 could not act on
```
