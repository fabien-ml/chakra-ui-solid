...
§3.5   L 397–409   0.6 KB  The `render` prop
...
```
§0   L 34–91   4.2 KB  The division of labour — this file, `CLAUDE.md`, and the nine documents beside it
...
```
§3.1 L  1–125 8.9 KB  P1 — identity, law, and the reference policy
```
...
```
§3.14 L  1–771 60.8 KB  S3b — the visual surfaces
```
...
## `decisions/3.18-declaration-support.md`
...
§1    L 43–68   4.1 KB  Per file
§2    L 69–105  6.3 KB  Per component
§3    L106–153 10.7 KB  Per batch
§3.0  L112–123  0.6 KB  The gate every batch shares
§3.1  L124–138  5.1 KB  The probe phase
§3.2  L139–153  4.8 KB  The batches
...
```
§0    L 32–71   3.4 KB  What this document specs, and where the rest of the site is
...
§3   L383–524 11.8 KB  The copyright boundary — every place upstream expression would be reproduced
...
§0.5  L 113–122   0.5 KB  Consequence for the reference policy
...
§9    L 970–1058  8.1 KB  Carry-over verdicts with attribution status
§9.1  L1018–1034  1.0 KB  Carry-overs the evidence no longer supports copying
§9.2  L1035–1058  1.4 KB  The port rule removes the repo's only Apache-2.0 obligation
§10   L1059–1141  9.4 KB  What P2 contradicts — for re-planning before P3
§10.1 L1064–1084  3.2 KB  The port rule — the widest change, and it came from the P2 gate, not the checkout
§10.2 L1085–1101  3.2 KB  Measurement contradictions
§10.3 L1102–1116  1.2 KB  Structural facts, not contradictions, that later phases should not assume away
§10.4 L1117–1132  1.1 KB  Confirmed unchanged, so nobody re-checks them
§10.5 L1133–1141  0.5 KB  Not reproducible from git, and not disputed
```
...
§3   L 255–449  11.7 KB  The generated-CSS coverage check — `check:css-coverage`
...
§3.7 L 426–438   2.2 KB  What it does **not** catch — five things, each with the artefact that does
...
§8   L 832–883  10.3 KB  The distribution, styling-config and structural checks
§9   L 884–936   5.2 KB  The attribution checks
§10  L 937–971   2.5 KB  The bundle measurement — `check:bundle`
§11  L 972–990   2.5 KB  The scheduled upstream checks
§12  L 991–1012  2.7 KB  The CI job map
§13  L1013–1029  1.0 KB  What this apparatus assumes
§14  L1030–1035  0.3 KB  Where this document ends
```
...
```
§8.3   L  1–152 10.3 KB  `check:context-budget` — the growth guard, in two halves
§8.3.1 L 25–110  5.6 KB  The static half
§8.3.2 L111–152  3.1 KB  The transcript half — `pnpm check:context-budget --sessions`
```

## `testing/8.04-declaration-support.md`

```
§8.4 L  1–170 11.2 KB  `check:declaration-support` — the emitted declaration, put to the engine
```
...
§1   L  60–168   8.1 KB  Q6 — copied or re-derived, answered twice
...
§3.5 L 319–339   1.5 KB  What that costs a future re-sync
...
§7   L 703–815   6.1 KB  The attribution checklist — lands in the same commit as the code
§7.1 L 718–755   1.6 KB  The seven headers
§7.2 L 756–786   1.9 KB  The `NOTICE.md` rows — and the number that matters
§7.3 L 787–815   1.8 KB  The checklist, in commit order
