...
| | `CLAUDE.md` | `decisions-ledger.md` (this file) | The other nine `__internal__` documents |
|---|---|---|---|
...
`decisions-ledger.md` D-01 and §3.5; P9's job at this gate was to make sure the answer is findable from the
...
**The anchors did not move when the bodies did.** `` `decisions-ledger.md` §3.13 `` still names the S3
...
| **3.12** | [S2 — the Zag adapter fork, milestone one](decisions/3.12-s2-zag-solid.md) | D-102 … D-110 | Step 2 closed: the fork at 87 tests, four deltas on two axes, and A1 re-diagnosed as a Solid 2.0 bug rather than a 1.x one |
...
**35 rows below, counted as rows rather than as decisions**, plus four in the table above. The two
...
| **`plan.md` §3.4** *(the `theme` half)* | What a consumer's re-declared key costs | `presets`, `staticCss` **and `theme`** are all hazards, and `mergeConfigs([chakraConfig(), { … }])` is the documented form — written at P3, restated in five more places, measured in none | **`theme` is safe under `extend`** and the merge form is a spread of a named call. `mergeConfigs` is not exported from `@pandacss/dev` and returns `any`; the surviving hazards are `presets`, and `staticCss` only when `responsive` was passed — D-177 |
...
**Status is a column, because a table that calls itself the single entry point and ticks only its
first row is how a reader loses their place** (S4, 2026-08-10). *Closed by* is the commit the step's
gate passed at; a step is ticked only when its `definition-of-done.md` §3.1 row is discharged.

| Step | What | Gate |
|---|---|---|
| **1** ✅ **closed by `190aa54`** | Repo bootstrap — workspace, catalog, Biome, tsconfig, Turbo incl. `codegen`, CI skeleton, three Vitest projects, `solid-contract` | `definition-of-done.md` §3.1 step 1 — the projects are distinguishable, 23 contract cases green incl. the three new `flush()` ones |
| **2** ✅ **closed by `046d0d2`** | the Zag adapter fork + the harness (D-48) | `zag-solid-adapter.md` §6.5's seven lines, verbatim; `definition-of-done.md` §3.1 step 2. **Six of seven green; the seventh, *A1 filed upstream*, is carried open and stays open** (**D-109**, **D-110**) |
| **3** ✅ **closed by `f6a339b`…`f0cfd38`** | The styling seam — Panda config, preset, `renderStyled`, style props, **plus the locale and environment contexts** | `definition-of-done.md` §3.1 step 3 — `Box`'s computed styles in all three projects, a consumer override changing them, and five checks live |
| **3b** ✅ **closed by `f57403b`…`2d54264`** | **The two visual surfaces, both rendering `Box`** — Storybook (local playground and compile-mode canary, **never deployed**) and the **docs app shell** with its own consumer `panda.config.ts` (D-98). **Split in two at the S3b review** (**D-128**): Storybook landed first, the docs app follows | `definition-of-done.md` §3.1 step 3b — **as amended by D-133**: Storybook contributes no gate line, `test:storybook` does not exist, and the `docs` CI job carries the whole step. **P8-B** and **P8-C** close; **P7-B** is retired unclosed |
| **4** ◀ **next** | One real slot recipe, in a throwaway consumer whose own source never names the variant | `plan.md` §1's gate; `definition-of-done.md` §3.1 step 4 — coverage green **there**, and flipping `hash` exits `E_CONFIG_MISMATCH` |
...
- `decisions-ledger.md` §6 item 1 — message the Chakra maintainers at first public release (D-03). Not an
  ...
  marked, not resolved, at `decisions-ledger.md` D-168. The trigger is unreached either way.
- `decisions-ledger.md` §6 item 8 — keep `@solid-chakra` held. Costs a renewal and nothing else.
...
| 7 | `decisions-ledger.md` D-168 + §6 above | *message the Chakra maintainers at first public release* | **D-110** (*"no maintainer contacted"*) | 2026-08-09 |
...
| `decisions-ledger.md` §6 item 2 | `legal.md` **§6** | open, assigned to P8 | **Closed at P8** — naming the Pages project is step one of the Cloudflare setup |
...
**One row found and deliberately not fixed.** `decisions-ledger.md` D-12 contains *"the definition of done was to
