...
— **Decision.** `chakra-ui-solid` ships `dist/panda.buildinfo.json` from `panda ship`, and
...
`defineChakraConfig(overrides)` (named) — a function returning a `defineConfig`-shaped fragment carrying every
...
adds a way to import half of what they need. *An object rather than a function* — `defineChakraConfig()`
...
— **Decision.** `defineChakraConfig({ responsive })` — omitted, `{ button: ["size"] }`, `["button"]`, or
...
**D-40 · Presence lives in `@chakra-ui-solid/core`, which therefore depends on `zag-solid`**
