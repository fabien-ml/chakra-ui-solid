...
`zag-solid`, went when the fork moved inside `core`. `check:resolution-sync` compared them until
`76382c5` deleted it; nothing does now, which is how a count this entry states could go stale
unreported.
