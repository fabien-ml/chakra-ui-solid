...
anywhere. *Add a base value in our preset* — that is a token table, which `CLAUDE.md`, *Reference use* forbids,
...
`@chakra-ui-solid/styled-system`, and none on `@chakra-ui-solid/core`.
...
— **Decision.** `system` declares no dependency on `zag-solid` at
...
shape Panda reads is the shape a consumer gets. The two files carry **four** resolutions each,
matching pairwise: `@chakra-ui-solid/core`, `chakra-ui-solid`, `@chakra-ui-solid/internal-test-utils`
and `@chakra-ui-solid/internal-test-utils/stylesheet`. It was five when this was written — a fifth,
`zag-solid`, went when the fork moved inside `core`. `check:resolution-sync` was never written, so
nothing compares the two files; `tsconfig.base.json`'s own comment and `apps/docs/vite.config.ts`
both name the script as though it runs.
— **Settled.** S3 gate, by reading the two files.
...
`definition-of-done.md` §6 and `CLAUDE.md`, the five obligations say the preset *"gains its first `NOTICE.md`"* with
...
— **Reasoning.** `CLAUDE.md`, the five obligations; `definition-of-done.md` §6; `testing.md` §9.
...
override. The pairing joins `testing.md` §11's lockstep table as a standing re-check on either
...
— **Decision.** `turbo.json`'s `cssgen` task declares the two source globs alongside
`$TURBO_DEFAULT$` — one entry each rather than a brace expansion, `$TURBO_ROOT$/packages/core/src/**`
and `$TURBO_ROOT$/packages/chakra-ui-solid/src/**` — and carries a comment naming the coupling: the
globs must stay in step with `packages/styled-system/panda.config.ts#include`.
...
package, so a style prop added in `packages/core/src` was a cache hit: `pnpm cssgen` printed
...
a file under `packages/core/src` and a cache hit before.
...
— **Decision.** `packages/core/src/style-props/style-props.tsx` →
`packages/core/src/render-styled/render-styled.tsx`, directory included. `html-props.ts` moves
...
`packages/core/src/render-styled/__tests__/`.
