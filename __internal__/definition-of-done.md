...
| 2.6 | ~~At least one story per part shape it uses, and every story opens~~ — **deleted at S3b** (**D-133**). Storybook is a local playground; no story is required to exist and nothing opens one. `prior-art.md` §8.1's rule keeps its force and changes its subject: **the deliverable that must be opened is the docs example**, and rule 2.15 plus `check:docs-examples` is where that now lives | — (deleted, not unenforced: see rule 2.15) |
...
| 2.15 | **A component is not done until its docs page is done** — complete, in the **same phase as the component**: anatomy, generated props tables, examples, the `ids` section (`docs-plan.md` §8's template). Not a stub backfilled later. Amended at the S1 review (**D-99**): this was an inventory check that could not fire until step 8, by which time every component had shipped without one. The docs site is the near-1:1 equivalent of `chakra-ui.com` for this library and is built **incrementally**, so it is readable as a site at every gate from 3b onward | `check:docs-inventory`, live from **step 3b** |
...
| **B3** — the field family | **A multi-part family with no machine** — the only one. Field's behavior is re-derived from the ARIA contract, and the mechanical proxy for *"never its expression"* is that `packages/chakra-ui-solid/src/components/field/**` has **no `@license` header and no `NOTICE.md` row**: if it needs one, the re-derivation failed (`CLAUDE.md`, *Reference use*) | A Field ARIA conformance suite written from the spec; atomic recipes composed *into* a slot recipe, proven by computed style; `input-group`'s runtime `calc()` converted to route 3 and held there by rule 1 |
...
| 4.9 | **The disclaimer is in every published package's README** — a publish-time gate, not a habit (`decisions-ledger.md` §6 item 7) | `check:readme-disclaimer` |
...
citations          `decisions-ledger.md` §3.16 — a Rejected line; blind spot 2 of `testing.md` §8.1 argues it
```
...
introduced at `decisions-ledger.md` §3.16 in the sentence that rejects it. The documented procedure has been
...
- **Rule 1.2** names `check:no-hand-written-data-attrs`, which does not exist. Its subject —
  `packages/chakra-ui-solid/src/components/**` — *does* exist and would pass trivially today, so the rule is
...
| **9** | **The `data-*` vocabulary diff** — spot-checked at 6 of 56 slot recipes, all matching. *The single cheapest check with the largest downside if skipped*: if it fails, every affected slot recipe needs a translation getter per part, a per-component tax nothing prices | **`check:data-attr-vocab`.** Tier 1 reads the emitting side from each machine's `.connect.ts`; tier 2 drives real machines through their states. The one known non-match is not one, **though not for the reason recorded until the `tabs` ship**: `_active` is `&:is(:active, [data-active])` and does select a `data-active` the machine never emits — but the recipe nests it inside `_disabled`, and a disabled `<button>` never enters `:active`. Dead CSS upstream, so a tier-1 hit on it is a false positive to allowlist rather than a translation to write | **Step 4** (tier 1), per batch (tier 2) |
...
| **P7-B** | **Retired at S3b, unclosed** (**D-133**). Measured first, and it holds — a runner drives a Solid 2.0 Storybook build and observes per-story console errors (**D-130**) — then withdrawn, because with no story gate nothing depends on the answer. Kept as a row rather than deleted: the measurement is what makes `testing.md` §7.2's warm-up a fact | — | — |
...
| **P8-C1** | The generator reads **our own part props** from the TypeScript compiler API with no running system object | **Closed at S3b**: a correct two-row table for `Box`, types and JSDoc read from `packages/chakra-ui-solid/src/components/box/box.tsx` | — |
...
| **P8-C3** | The generator reads a **machine's `Props` type** through our lockfile | ~~A non-empty, correct Root props table for Dialog~~ **The gate stopped discriminating.** Collapsible shipped the first machine page with a non-empty, correct Root table and input 1 still unbuilt — `CreateCollapsibleProps` declares the machine's consumer-facing keys itself (`docs-site.md` §4.2) | **Open**, and blocking nothing |
...
> row 5 → `decisions-ledger.md` §6 items 6 and 7 · row 6 → `CLAUDE.md`, the five obligations · row 7 →
> `component-blueprint.md` §1.3 · row 8 → `plan.md` §0.4 · row 9 → `plan.md` §12 row 3. Row 2 was
> P8's and is discharged in `docs-plan.md` §1.2. The full log is `decisions-ledger.md` §7; this table stays
...
| **5** | `decisions-ledger.md` §6 items 6 and 7 are open, assigned to P7 | **Closed here, by name:** `check:license-headers`, `check:notice-rows`, `check:package-files` for item 6; `check:readme-disclaimer` as a **publish-time** gate for item 7 | **P9** — `legal.md`'s open-items table |
