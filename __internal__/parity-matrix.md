...
**Moved to [roadmap.md](roadmap.md)**, where each row's note sits under the row it belongs to and
the checkbox tracks whether it is built. The `Status`/`Batch` columns this section carried are
dropped — every row shipped is a checked box there.
...
| **3b** | **The two visual surfaces**, both rendering `Box` — Storybook (a local playground, **never deployed**) and the **docs app shell** with its own consumer `panda.config.ts`. Added at the S1 review, **D-98**; split into two commits at the S3b review, **D-128** | Both run. **Amended by D-133**: Storybook has no gate and no CI job, so the `docs` job is the whole of it; **P8-B** and **P8-C** close, **P7-B** is retired unclosed |
