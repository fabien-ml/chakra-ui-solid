...
> record is `decisions-ledger.md`.

**Two things settled earlier and not reopened here.** The brand is `chakra-ui-solid`, scope
`@chakra-ui-solid` (`decisions-ledger.md` D-01). The **port rule** is *no accessibility behavior beyond what
...
@chakra-ui-solid/styled-system/recipes   ← generated: one exported function per recipe
        ↓ static import
@chakra-ui-solid/core                  ← the slot-recipe context: resolves once per Root,
...
B**: `chakra-ui-solid` ships `dist/panda.buildinfo.json`, produced by `panda ship`, and
...
@chakra-ui-solid/core              PUBLIC, hand-written. §5.3. Also holds the Solid 2.0 fork of
                                   @zag-js/solid at src/zag/ — 7 source + 7 test files, planned as
                                   its own package and absorbed here instead. ← milestone one.

chakra-ui-solid                    PUBLIC. The components.
                                   ^ P — no buildinfo. The consumer channel became a glob over
                                     dist/**/*.jsx (§3.4); the only `ship` script left is
                                     styled-system's, and nothing invokes it.
...
> gate; the rest are **I**.
>
> The graph **as it actually stands today**, measured from the manifests — and `zag-solid` is no
> longer one of the nodes, having been absorbed into `core` as `src/zag/`:
> `panda-preset → nothing`, `styled-system → panda-preset` (dev),
> `core → styled-system`, `chakra-ui-solid → core + styled-system`,
> `internal-test-utils → styled-system` (dev), `apps/docs → chakra-ui-solid + styled-system`.

| Package | Depends on, in-repo | Today |
|---|---|---|
| `panda-preset` | nothing | **I** — and the package is `panda-preset`, not `preset`, since **D-161** |
| `zag-solid` | nothing | **Not a package.** Absorbed into `core` as `src/zag/`, so both `zag-solid` edges below are now internal to one manifest |
| `styled-system` | `panda-preset` (dev/config-time only — Panda reads it to generate) | **I** |
| `core` | `styled-system`, `zag-solid` | `styled-system` **I**. **`zag-solid` is P — the edge does not exist. Gate: step 5**, when the presence render strategy lands, because the edge exists *because presence is a machine* (**D-40**, **D-119**) |
| `chakra-ui-solid` | `core`, `styled-system`, `zag-solid` | `core` and `styled-system` **I**. **`zag-solid` is P — same gate, same reason** |
| `internal-test-utils` | `core` — **from milestone 3, not before** (see below) | **P — still not there at step 3b.** The edge today is `internal-test-utils → styled-system`, which this table does not list. Gate: the first test helper that renders something styled |
| `apps/docs` | `chakra-ui-solid` | **I**, and it also depends on `styled-system` directly — the docs app is a *consumer instance* (`docs-site.md` §1.1), so it takes the generated surface the way any consumer does |

`zag-solid` depends on nothing here — that is what lets it be milestone one and ship before any
styling decision is settled. `core` depending on `zag-solid` is new relative to the plan's graph and
...
### 5.3 What `@chakra-ui-solid/core` owns
...
**Presence lives in `@chakra-ui-solid/core`.** Three reasons: at least six components need it
...
> **Amended at S3b, 2026-08-09 (`decisions-ledger.md` D-134), reversing D-38.** The measurement below is
...
**Measured at step 3, and the class is mandatory rather than an opt-in** (`decisions-ledger.md` **D-113**).
...
| **7** | npm scope availability | **CLOSED at P1.** `@chakra-ui-solid` is owned (`decisions-ledger.md` D-01) | — |
...
not in `@chakra-ui-solid/core` (§5.3), and presence is a build over a machine rather than a
carry-over (§6).
