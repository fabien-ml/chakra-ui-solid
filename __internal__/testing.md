...
| A rule that exists **with one declaration dropped** — the preset's `cursor: "switch"` against a token registered as `swittch` (`roadmap.md` §1.3c) | The class exists and has a rule. One property inside it is gone | Three, in widening order: a computed-style assertion (§2.2); `check:preset-token-resolution` for that one token (assumption **P6-F**); and **`check:declaration-support` (§8.4) for the general case** — every declaration in the sheet put to `CSS.supports` in a real engine, which is how the other eight of this shape were found |
...
| **8.4** | [`check:declaration-support` — the emitted declaration, put to the engine](testing/8.04-declaration-support.md) | Where the oracle runs and what was rejected, the canary, the allowance register, and what the first run found in our own source and in the preset |

**Why this section is sharded and §1–§7 are not.** §8 is a container of independently-citable units
that grows by one every time a check is written — §8.4 arrived one commit after the shard did, which
is the argument demonstrated rather than predicted — and `definition-of-done.md` §7b names **25**
still to come, most of them owing a definition here. It reached 16.7 KB carrying **two** definitions,
and §8.3 is 10.2 KB: unsharded, this section would be **~27.5 KB — over the ceiling on the very
commit that introduces it.** The other sections are single arguments that do not grow with the build.
This is `decisions-ledger.md` §3's situation one document over, and it takes §3's answer — the machinery
...
`decisions-ledger.md` §6 items 6 and 7 assign these here. **Both failure modes are silent and green**, which is
...
| `check:readme-disclaimer` | **Every published package's README** carries the disclaimer, not just the root's | `decisions-ledger.md` §6 item 7, and the reason it is a check: a per-package README is written once per package and then never looked at again. It runs at **publish time** — a `prepublishOnly` gate plus a release-workflow job — so a new package cannot ship without it |
...
| `styling` | every push, after `codegen` + `cssgen` | `check:css-coverage`; `check:coverage-allowlist`; `check:anatomy-parts`; `check:hash-config`; `check:preflight-hidden`; `check:data-attr-vocab`; `check:style-prop-collisions`; `check:no-hand-written-data-attrs`; `check:alias-coverage`; `check:dark-selector`; `check:preset-token-resolution`; `check:declaration-support` — **the only job that installs a browser outside the `test` matrix**, because its oracle is `CSS.supports` in a real engine (§8.4) |
| `docs` | every push, after `codegen` + `cssgen` | **Live from S3b:** the docs build (prerendered, not an SPA shell); `check:docs-inventory`; `check:docs-consumer-config`; `check:docs-examples`. **Joining at step 4:** `check:extraction-fixture`, `check:docs-links`, `check:docs-no-server-fns`, `check:docs-forbidden-claims`, and `check:css-coverage` against the docs app's own sheet — which is the step it first has a buildinfo to read (`decisions-ledger.md` **D-139**). A deploy step on PRs and on the release branch. **Added at P8** (`docs-site.md` §6.1, §8 row 2) — the docs app is a standing instance of the step-4 consumer gate, so its build failing is a distribution failure, not a documentation one |
