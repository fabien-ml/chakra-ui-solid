...
/**
 * The page frame, in one module because four components have to agree about it.
 *
 ...
 * (`docs-site.md` §1.1). So the same two properties are declared on the `<body>` class instead,
 * which is an ordinary Panda rule in this app's own sheet.
 *
 * One height at every breakpoint, where theirs grows to 104px from `md`: their header has a
 * second row for picking a docs section, because their first row is a site-level nav over five
 * content types. This site has one, so the sections are the top bar and there is no second row.
 */
