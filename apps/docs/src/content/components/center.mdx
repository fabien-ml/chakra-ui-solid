...
`Center` is a single factory call with one variant, `inline`, and no props of its own beyond it, so
there is no generated table here. It takes the whole style-prop surface, plus `as` and `render`.
