...
The React version composes `Avatar` and `Badge`. `Avatar` has not shipped here yet, so a `Circle`
with initials stands in for it. What the example is about — a `Float` anchored to the corner of a
