...
body declares one, discarding what the config asked for — so a config-level rule for a recipe that
also has a body rule is dropped before it is read. Rules you write under `staticCss.recipes` are
moved to the body for you, so both spellings work:
...
Panda run prints what it found:

```txt
🐼 chakra-ui-solid  4 components → 4 recipes: Button, Dialog, Field, Table
```
