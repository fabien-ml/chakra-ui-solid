...
That one needs the merge more than `theme` does, and `extend` is no help. A config's
`staticCss.css` **replaces** a preset's whole array, and writing it as `staticCss: { extend: { css } }`
replaces it too — the `extend` escape rescues `theme` and does nothing here. So the preset's own
seven entries are written into your config alongside yours rather than left behind in the preset,
and an `extend` block of yours is folded into the same union. Either spelling works and neither
loses anything.
