...
// Both of this app's **own** Panda outputs, written by `pnpm --filter @chakra-ui-solid/docs cssgen`
// and `codegen` from `panda.config.ts`. Not the repo's dev stylesheet or its generated runtime —
// those are produced from *our* source for the browser tests and Storybook, and reaching for either
// here would make the site stop being evidence that a consumer's build works (`docs-site.md` §1.1).
//
// The class names the components below compute and the rules in this sheet therefore come out of
// one Panda run over one config, which is what a reader is told to do on the install page.
...
function RootLayout(props: { children: JSX.Element }) {
  ...
  return (
    // Above `<html>`, because every route is prerendered: a component below this that could not
    // read the system would fail the build rather than only the dev server.
