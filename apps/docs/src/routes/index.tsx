...
function DocsHome() {
  return (
    <div class={css({ maxW: "3xl", mx: "auto", px: "6", py: "16" })}>
      <h1 class={css({ fontSize: "5xl", fontWeight: "bold", letterSpacing: "tighter" })}>
        {SITE_NAME}
      </h1>
      ...
      <p class={css({ mt: "4", fontSize: "lg", color: "fg" })}>
        It is <strong class={css({ fontWeight: "semibold" })}>{PARITY_SENTENCE}</strong> — not a 1:1
        port, and it does not pretend to be one.
      </p>
      ...
      <Section title="Before you install">
        <ul class={css({ listStyleType: "disc", pl: "6", color: "fg.muted" })}>
          <li class={css({ my: "2" })}>
            <strong class={css({ color: "fg" })}>SolidJS 2.0.</strong> Not 1.x — the packages import{" "}
            <code>@solidjs/web</code>, and a 1.x install fails at load.
          </li>
          <li class={css({ my: "2" })}>
            <strong class={css({ color: "fg" })}>{PANDA_PREREQUISITE}</strong> Your Panda run reads
            your source and writes your stylesheet; there is no supported way to use this library
            without one.
          </li>
          <li class={css({ my: "2" })}>
      ...
      <Section title="What you get, and what you do not">
        <div class={css({ display: "grid", gap: "6", md: { gridTemplateColumns: "2" } })}>
          <div>
...
function Section(props: { title: string; children: unknown }) {
  return (
    <section class={css({ mt: "12" })}>
      <h2 class={css({ fontSize: "xl", fontWeight: "semibold", color: "fg", mb: "3" })}>
        {props.title}
      </h2>
