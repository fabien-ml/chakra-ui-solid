...
import type { CreateDrawerReturn, DrawerRootBaseProps } from "./drawer.types";
 ...
 *
 * `drawerSlotNames` lists **eleven** entries — `backdrop` appears twice. The duplicate is
 * source-only: both entries build the same `drawer__backdrop` class into the same key, so the
 ...
 * {@link CreateDrawerReturn}'s getters) instead of spreading it.
 */
export interface DrawerContextValue extends CreateDrawerReturn {
  /** One class string per slot, resolved once on the Root. */
  slots: Accessor<Record<DrawerSlot, string>>;
  /**
...
/**
 * Distinct from Dialog's, even though the two hold the same machine. In React both names resolve to
 * Ark's single dialog context, so upstream's `DrawerActionTrigger` can call `useDialogContext()`;
 * here they are two Solid contexts and a Drawer part reading Dialog's would throw.
 */
export const [DrawerProvider, useDrawerContext] =
  createComponentContext<DrawerContextValue>("Drawer");

/**
 * The slot classes the Root resolved, for an element of your own inside a Drawer:
 *
 * ```tsx
 ...
 * machine-less rows' `useStyles` hands back, and it throws outside a `Drawer.Root` for the same
 * reason they do.
 */
export const useDrawerStyles = (): Accessor<Record<DrawerSlot, string>> => useDrawerContext().slots;

export const { PropsProvider, usePropsContext } = createPropsContext<DrawerRootBaseProps>();
