...
describe("renderElement", () => {
  ...
  it("forwards the computed props onto the element the `render` function returns", async () => {
    // The reason `RenderProp` is function-only: a JSX *element* would be an already-built DOM node,
    // and every computed prop below (onClick, aria-*) would be silently dropped.
    const onClick = vi.fn();
    const { dispose } = mount(() =>
      renderElement({
        as: "button",
