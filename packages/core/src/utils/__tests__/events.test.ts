// Carried from hope-ui `main` (1dc059f), `packages/primitives/src/utils/__tests__/events.test.ts`,
...
describe("composeEventHandlers", () => {
  ...
  it("stops once a handler calls preventDefault", () => {
    // The cancel channel: a consumer's `onClick` runs first and can veto the component's own
    // behavior (`Dialog.Trigger`'s `setOpen(true)`). It is the one thing this adds to React Aria's
    // `chain`, and the only reason a component would reach for it over plain composition.
    const internal = vi.fn();
    const composed = composeEventHandlers<HTMLButtonElement, Event>(
