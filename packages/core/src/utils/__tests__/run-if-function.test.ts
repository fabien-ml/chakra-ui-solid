...
describe("runIfFunction", () => {
  ...
  it("invokes the factory once per call, yielding a fresh result each time", () => {
    // The whole point of the factory form: a single shared factory produces an independent value on
    // every call, so two consumers never fight over one already-built subtree. A Solid `JSX.Element`
    // is a real DOM node, and reusing one *moves* it.
    let count = 0;
