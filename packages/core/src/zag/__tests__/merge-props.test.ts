...
describe("mergeProps", () => {
  ...
  it("combines data-ownedby tokens", () => {
    // The tripwire fired, exactly as the fork armed it. At the `1.42.0` this was pinned to,
    // `@zag-js/core`'s `mergeProps` had no union branch for `data-ownedby` and the composing
    // branch fell through to last-value-wins, so this assertion read `"tooltip popover"` with a
    // comment saying a core bump is what should turn it red. `1.43.0`'s `53944e0` added the
    // branch — `ownedBy(…)` — and `@zag-js/dom-query`'s `getByOwnerId` now selects
    // `[data-ownedby~="…"]`, a *token* match, so the union is what the DOM query needs. The
    // source needed no change: the delegation was written for this (`zag-solid-adapter.md`
    // §4.3 D2). Same expectation as upstream's own case, now.
