...
// importing it from there fails at load. It lives in `solid-js`.
 ...
 * undocumented `@solidjs/web` internals, each naming the code that depends on it — but these must
 * run where `@solidjs/web` resolves to `dist/web.js` rather than `dist/server.js`, which is only
 * true in the **browser** project (`testing.md` §1.3).
 */
...
describe("@solidjs/web client-build contract", () => {
  describe("applyRef flattens a ref array and skips falsy entries", () => {
    ...
    // itself an array — is a non-issue, which is why **no `mergeRefs` helper exists**. If stable
    // stops flattening or stops skipping falsy entries, one is needed.
  ...
  describe("sharedConfig.hydrating marks the hydration pass, and only it", () => {
    // The flag any "am I hydrating right now?" branch reads. In hope-ui that was the i18n module's
    // locale detection, which reported the server's `en-US` for exactly as long as this flag was
    // set — the gate that makes zero-config i18n SSR-safe without costing a client-only app an
    // en-US first paint. The equivalent gate here is the locale/environment context of
    // `plan.md` §7.2, which is step 3 work and not yet written; the flag is pinned now because
    // 1.x spelled it `sharedConfig.context`, and a rename or a narrowing of its coverage is
    // silent — the value simply disagrees with the server's markup again.
  ...
  describe("a signal write from one document listener cannot unhook the next one mid-dispatch", () => {
    // The observable form of divergence 4's premise, at the place it actually bites: a `document`
    // listener that writes a signal during dispatch.
    //
    // Zag's dismissable layer stack is built this way — every open layer attaches its own
    // capture-phase `pointerdown` listener from its own effect, and the topmost layer's handler
    // dismisses, which writes the very state those effects track. That is only safe if a dispatch
    // cannot be reordered underneath itself: were the write to re-run the effects mid-dispatch, a
    // lower layer's listener would be detached before the event reached it — or a re-attached one
    // would see it twice — and the guard would be reading a stack that moved under it.
    //
    // Solid defers the re-run to the next flush, so it cannot. The adapter's `send` depends on the
    // same deferral (`zag-solid-adapter.md` §3.4, divergence 4). If this ever stops holding, every
    // single-phase outside-pointerdown guard in the closure needs a two-phase snapshot instead.

    const PROBE_EVENT = "chakra-ui-solid-contract-probe";

    /**
     * Two sibling effects, each attaching a `document` listener while `openLayers` is high enough
     * to keep its layer open — the shape a dismissable layer stack produces per layer. The upper
     * one's handler dismisses itself by writing that same signal.
     */
