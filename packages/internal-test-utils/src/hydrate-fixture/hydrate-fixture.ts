...
export interface HydrateFixtureOptions {
   ...
   * settles, where replacing nodes is the feature under test rather than a failure. **No tree in
   * this repo needs it yet**; the shape that does is a locale provider rendered with no `locale`
   * prop, which serves the server's `en-US` and then adopts the visitor's locale, rebuilding every
   * locale-derived node (hope-ui's `I18nProvider`, the case the option was written for — the
   * equivalent here arrives with the locale context at step 3). The console-silence and
   * element-count checks still run, and are what still tells a re-render apart from a fallback.
   */
