...
/**
 * Owns the signal, and reflects whatever gets registered onto its own `aria-describedby` — the real
 * shape of the pattern, and how the test observes it. `Child` is a separate component, so its writes
 * cross a reactive-scope boundary.
 */
...
describe("createRegisteredId", () => {
  ...
  it("does not throw [REACTIVE_WRITE_IN_OWNED_SCOPE] writing to an ancestor's signal", () => {
    // The reason this is a primitive rather than a snippet. A plain `props.register(props.id)` in
    // `Child`'s body is a descendant writing to an ancestor-owned signal, which SolidJS 2.0 throws
    // on. `createRegisteredId` defers the write into `onSettled`, out of that call stack.
