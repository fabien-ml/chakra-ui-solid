...
export function useMachine<T extends MachineSchema>(
  ...
  onCleanup(() => {
    // Upstream's guard, from `1.43.0`'s `42d8a92` (#3228) — "skip machine exit actions when a
    // component is disposed before the machine starts". `onSettled` is where the machine starts,
    // so a tree disposed before it runs would otherwise fire root exit actions that never had a
    // matching entry (`zag-solid-adapter.md` §4.3 D1).
