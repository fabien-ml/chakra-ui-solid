 ...
 *
 * `data-ownedby` is listed because `@zag-js/core@1.43.0` gives it a **union** branch: it composes
 * the owners as space-separated tokens, and `@zag-js/dom-query`'s `getByOwnerId` selects them with
 * `[data-ownedby~="…"]`, a token match. Routing the key here is what delegates to that branch. At
 * the `1.42.0` this fork was taken from, core had no such branch and the same delegation yielded
 * last-wins — `merge-props.test.ts` pinned that, and turned red on the bump exactly as intended
 * (`zag-solid-adapter.md` §4.3 D2).
 */
